...
Let's say we have $p$ observed variables, denoted as $X_1, X_2, ..., X_p$. The factor model for the $i$-th observed variable ($X_i$) for a single individual can be expressed as:

$$X_i = l_{i1}F_1 + l_{i2}F_2 + ... + l_{im}F_k + \epsilon_i$$
...
* $\epsilon_i$ is the error term (or uniqueness) associated with the $i$-th observed variable. It represents the variance in $X_i$ that is not explained by the common factors. These are assumed to be uncorrelated with each other and with the common factors, and have a mean of 0.
