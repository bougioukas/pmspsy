...
The final step in conducting a systematic review is to synthesize the results. This synthesis can be either qualitative or quantitative, if the data allows for appropriate statistical synthesis (i.e., meta-analysis).

If meta-analysis is not feasible, authors should describe in a narrative way and summary tables the findings by grouping them according to interventions, population, outcomes, or study design.

For systematic reviews of interventions, for each study a summary statistic is calculated, which is either a (standardized) difference in means for continuous outcomes, a risk ratio (RR) or an odds ratio (OR) for dichotomous outcomes. Subsequently, an overall effect estimate is generated as a weighted average of the effects estimated in the individual studies.

Every single trial is different and studies brought together in a systematic review will inevitably differ to some extent. Reviewers should assess heterogeneity, which may arise from factors such as participants' characteristics, disease severity, concomitant treatments, or methodological quality. Note that there are two basic meta-analytic options based on the presence of heterogeneity: the fixed-effect model and the random-effects model.
