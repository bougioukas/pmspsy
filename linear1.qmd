...
![Assumption Checks choices.](images/scat7.png){#fig-scat7 width="65%"}
...
Now, let's focus on interpreting the regression table in @fig-scat9. In the **estimate** column are the intercept $b_o$ = `r format(evals_line[1], digits=6)` and the slope $b_1$ = `r format(evals_line[2], digits=5)` for height. Thus the equation of the regression line becomes:
...
Of greater interest is the slope of height, $b_1$=$`r format(evals_line[2], digits=5)`$, as it summarizes the association between the height and weight variables.
...
-   The intercept corresponds to the mean weight 4258 g for a singleton infant which is the reference category.
