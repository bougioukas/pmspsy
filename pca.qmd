...
PCA seeks a small number of dimensions that are as interesting as possible, where the concept of interesting is measured by the amount that the observations vary along each dimension. Each of the dimensions found by PCA is a linear combination of the $p$ observed variables. 


It is important to note that there are $p$ possible principal components representing the full-dimensional space of the data. The first principal component of a set of observed variables $X_1, X_2, . . . ,X_p$ is the normalized linear combination of the observed variables:
...
-   **Determine the Number of Components:** Identify how many components to extract, guided by criteria such as eigenvalues greater than one, scree plot inspection, or parallel analysis. The extracted components typically represent underlying dimensions that group the observed variables. By selecting the appropriate number of components, we can ensure that the analysis captures the most important sources of variance in the data.

-   **Extract and Rotate Components:** Extract the principal components based on the selected number. Consider applying an orthogonal rotation (e.g., varimax) to clarify the component structure. Keep in mind that rotation is optional in PCA and affects interpretability but not the total variance explained. Once the component structure is finalized, compute the component scores for further analysis, such as visualization, clustering, or regression.
...
-   “Meaningfulness criterion” Theoretical justification – Retain components that align with prior knowledge, conceptual frameworks, or hypotheses relevant to the domain of study (fixed number).
...
In our example we will retain the first four components for further analysis.
...
It is important to note that in PCA with **orthogonal** rotation (such as Varimax), the component loadings can be interpreted as **correlation coefficients** between the original observed variables and the rotated components.
