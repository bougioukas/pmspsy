...
In PCA, the principal components are orthogonal (uncorrelated) by definition and are ordered based on the amount of variance they explain. While rotation is not necessary for PCA from a mathematical standpoint, it can be particularly useful when the goal is not only to reduce dimensions but also to interpret the components in a practical and meaningful way, as in our case.
...
-   **Orthogonal Rotation:** Use this if you believe the components are independent or unrelated (it preserves the orthogonality property of classical PCA).
    -   Varimax is the most common orthogonal rotation method.
    
-   **Oblique Rotation:** Use this if you believe the components are correlated or related (although, at this point, exploratory factor analysis (EFA) may be a more technically appropriate model).
...
After rotation, there are four clear components/scales (**NOTE:** In the unrotated method, that is "Rotation None" in Jamovi, most variables loaded on the first component).
...
The component analysis diagram follows:
...
In PCA, the components are considered orthogonal (uncorrelated), and applying an oblique rotation may not align with the standard objectives of PCA. However, oblique rotations (such as Promax or Oblimin) are available for PCA in Jamovi, despite the resulting loss of strict PCA properties. Allowing components to correlate can sometimes aid interpretation, particularly when the goal is to better model or understand associations among variables that are not fully independent, as is often the case in real-world psychological research.
...
Note that this component analysis diagram includes links with numerical values **between components**, representing the correlation coefficients among them (@fig-mlinear38). In contrast, orthogonal rotation assumes the components are uncorrelated, so such links do not appear.
...
New variables have been created, each containing the component scores for the extracted components (@fig-mlinear39, @fig-mlinear401).

![Component scores after Varimax rotation.](images/mlinear39.png){#fig-mlinear39 width="65%"}


![Component scores after Oblimin rotation.](images/mlinear401.png){#fig-mlinear401 width="65%"}