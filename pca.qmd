...
In PCA the components are considered orthogonal (uncorrelated), and rotating them in an oblique way may not align with the standard objectives of PCA (e.g. dimensionality reduction). However, oblique rotations (such as Promax or Oblimin) are available in Jamovi for PCA. These rotations allow the components to become correlated, which may sometimes make the interpretation of the components more meaningful, especially when we are interested in modeling or understanding the association between variables that are not strictly independent.
