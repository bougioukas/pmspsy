# Introduction to Jamovi {#sec-introduction}
...
(**NOTE:** You can find other formats of the data (OMV, Excel) at the link: [https://osf.io/gvctz/](https://osf.io/gvctz/)).
...
We can assign labels to numerically coded values of categorical variables, such as `sex`, by accessing the variable settings. One way to achieve this is by double-clicking on the variable name `sex`, which  opens the additional menu of variable settings at the top of the Jamovi screen (@fig-labels0).
...
Notice how the numbers "1" and "2" have moved to the lower right under the text we're typing, allowing us to still see which label corresponds to each number. Click anywhere outside the labels box to save these labels.

We close the variable settings by pressing the arrow in the top-right corner ![](images/up_arrow.png){width="25" height="25"}.

 

### Changing the measure type and adding labels

First, we will add labels to the nominal variable `sex`.  




the measure type from nominal to continuous for all variables except `sex`. For example, double-click on the variable name `age`. This will open an additional menu at the top of the JAMOVI screen, as shown in @fig-type_age:
