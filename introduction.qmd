...
Click on `time_spent` variable and then press the ![](images/transform_button.png){width="36" height="34"}) from the toolbar of the Data tab.

This opens the "**TRANSFORMED VARIABLE**" view where we can set the name of the transformed variable such as `time_spent2` and create the transformation by selecting the source variable, here the `time_spent`, in the *Source Variable* field, and creating a new transformation using transform field (@fig-create_new_transf).


![Setting up the transformation of a variable.](images/create_new_transf.png){#fig-create_new_transf}
...
![Box for adding conditions to the transformation.](images/transf_view.png){#fig-transf_view}
...
-   In the next box, we will put the `time_spent` value we want as the cut off point.
...
![Adding the first condition.](images/recode2.png){#fig-recode2}


Finally, after the `else use` box just add the label for the data that does not meet the above conditions (here `'obese'`), as shown in @fig-recode3.

![Adding the first condition.](images/recode3.png){#fig-recode3}
