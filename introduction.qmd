# Introduction to Jamovi and data preparation {#sec-introduction}
...
Finally, we will change the measure type of `q1` to `q10` variables from nominal to ordinal. In the Variables tab, we tick the `q1` to `q10` variables, as shown in @fig-q1_10.
...
Click on `time_spent` variable and then press the ![](images/transform_button.png){width="40" height="36"}) from the toolbar of the Data tab.
...
![Adding the first condition.](images/recode1.png){#fig-recode1}
...
![Adding the second condition.](images/recode2.png){#fig-recode2}

 

Finally, after the `else use` box just add the label for the data that does not meet the above conditions (here `'>11'`), as shown in @fig-recode3.

![Adding the final label for the data that does not meet the above conditions.](images/recode3.png){#fig-recode3}
 

-   **Reverse scoring of items**

In order to compute a total score for all items of RSES we need first  to reverse the score of the negatively worded questions 3, 5, 8, 9, and 10. To reverse score these items, we can just use the following simple steps.

In the Variables tab, we tick the `q3`, `q5`, `q8`, `q9`, and `q10` variables, as shown in @fig-rev_items.
...
In the transform window, we enter `_R` as a suffix for the variable names and specify `3-$source` in the condition box, as shown in @fig-new_transform2.

![Transform variable settings.](images/transform2.png){#fig-transform2 width="60%"}

![Data with the reversed variables.](images/reversed_data.png){#fig-transform2 width="60%"}
...
Finally, we will compute the total score (0-30) from responses to 10 questions on the Rosenberg Self-Esteem Scale.

Adding computed variables to a Jamovi spreadsheet is straightforward.
Click on the Compute button ![](images/compute_button.png){width="40" height="36"}) from the toolbar of the Data tab. An empty column has been created at the end of our dataset (@fig-compute1). The black dot symbol in the right of the column header indicates that this is a computed variable.

![Compute a new variable.](images/compute1.png){#fig-compute1 width="60%"}

To set up the computed variable, either double-click the column header, or click the **Setup** button ![](images/setup_button.png){width="36" height="34"}) in the **Data** tab. This opens the "**COMPUTED VARIABLE**" view, where we can name the computed variable `score`. Next, we select the `SUM` function from the available options for use in the **Formula** box. Finally, we add the `q`-variables in the formula separated by comma (note that we must use the reversed variables in the sum) (@fig-sum_score) and we press `ENTER`:

![Compute the total score.](images/sum_score.png){#fig-sum_score}
